#!/usr/bin/env python3
"""Seed Georgia-metro demo contractors and published portfolio projects.

Feeds the public API consumed by portfolio_website/ (and red_clay_website/).

Usage (from JOBPULSE repo root or api/):
  cd api && PYTHONPATH=. .venv/bin/python scripts/seed_portfolio.py
  # or: make portfolio-seed
...
CONTRACTORS = [
    {
        ...
        "projects": [
            ...
            },
            {
                ...
                "summary": (
                    "Swapped a builder-grade vanity for a double sink unit in Inman Park. "
            ...
            },
            {
                ...
                "summary": (
                    "Kitchen cabinet refresh in Marietta with soft-close hardware upgrades "
                    "and careful rehang of doors and drawers for a quieter, cleaner finish."
                ),
                "service_key": "kitchen_cabinets",
                "city": "Marietta",
                "state": "GA",
...
async def seed() -> None:
    async with AsyncSessionLocal() as db:
        ...
        for spec in CONTRACTORS:
            ...
            user = User(
                email=email,
                full_name=spec.get("owner_name") or f"{spec['name']} Owner",
            ...
            for project in spec["projects"]:
                private_title = project.get("private_title") or f"PRIVATE {project['title']}"
                job = Job(
                    company_id=company.id,
                    created_by=user.id,
                    title=private_title,
