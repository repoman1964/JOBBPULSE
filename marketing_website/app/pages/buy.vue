...
const paypalLink = (config.public.paypalPaymentLink as string).trim()
const stripeReady = computed(() => Boolean(stripeLink))
const paypalReady = computed(() => Boolean(paypalLink))
...
useSeoMeta({
  title: 'Buy JobbPulse',
  description: `Get ${offer.productName} for ${offer.priceLabel} ${offer.pricePeriod}. Pay with card or PayPal. Covered by the 90-Day Work Guarantee and 2-Day Publishing Promise.`,
})
...
function onSubmit(event: Event) {
  ...
  if (method === 'paypal' && paypalLink) {
    persistCheckout(buyer, 'paypal', offer.sku, offer.priceAmount)
    ...
    <div class="container buy-wrap">
      <div class="buy-intro">
        ...
        <div class="buy-leads">
          ...
          <p>90-Day Work Guarantee · 2-Day Publishing Promise</p>
        </div>
      </div>
          ...
          <div class="buy-card-body">
            ...
            <div class="buy-guarantee">
              ...
              <div>
                ...
                <a class="buy-guarantee-link" href="#guarantees">Read the full guarantees</a>
              </div>
            </div>
          </div>
        </aside>
          ...
          <form class="buy-form" novalidate @submit="onSubmit">
            ...
            <fieldset class="buy-pay">
              ...
              <button
                ...
                :aria-describedby="paypalReady ? undefined : 'paypal-unavailable'"
              >
                <span class="pay-icon pay-icon-paypal" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="currentColor">
                    <path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944 3.72a.77.77 0 0 1 .76-.653h6.9c2.286 0 3.96.55 4.976 1.634.94 1.005 1.314 2.38 1.11 4.082-.012.1-.026.2-.042.3-.62 3.536-2.7 5.14-6.18 5.14h-1.82a.9.9 0 0 0-.889.76l-.75 4.76a.64.64 0 0 1-.633.554zm9.45-13.16c-.06.36-.14.72-.25 1.1-.89 3.42-3.14 4.66-6.3 4.66H8.37l-.86 5.46h2.39c.41 0 .76-.3.82-.7l.03-.18.64-4.07.04-.23a.83.83 0 0 1 .82-.7h.52c3.35 0 5.97-1.36 6.74-5.29.32-1.64.15-3.01-.7-3.98a3.3 3.3 0 0 0-.3-.27c.39.94.45 1.94.2 3.2z" />
                  </svg>
                </span>
                <span class="pay-copy">
                  <span class="pay-title">Pay with PayPal</span>
                  <span class="pay-sub">PayPal balance or linked cards</span>
                </span>
                <span class="pay-go" aria-hidden="true">→</span>
              </button>
              <p v-if="!paypalReady" id="paypal-unavailable" class="buy-hint">
                PayPal is not connected yet. Set
                <code>NUXT_PUBLIC_PAYPAL_PAYMENT_LINK</code>
                before launch.
              </p>
            </fieldset>
      ...
      <article id="guarantees" class="buy-guarantee-full">
        <h2>{{ workGuarantee.title }}</h2>
        <div class="buy-guarantee-copy">
          <p v-for="(line, index) in workGuarantee.paragraphs" :key="`work-${index}`">
            <strong v-if="line.emphasize">{{ line.text }}</strong>
            <template v-else>{{ line.text }}</template>
          </p>
        </div>
        <h3>{{ publishingPromise.title }}</h3>
        <div class="buy-guarantee-copy">
          <p v-for="(line, index) in publishingPromise.paragraphs" :key="`pub-${index}`">
            <strong v-if="line.emphasize">{{ line.text }}</strong>
            <template v-else>{{ line.text }}</template>
          </p>
        </div>
