...
const locationLabel = computed(() =>
  cityOnly(page.value?.location?.city, page.value?.location?.name) || 'this area',
)

useSeoMeta({
  title: () => page.value?.title || 'Local projects | JobPulse',
  description: () =>
    `${page.value?.project_count || 0} documented ${page.value?.service?.name || ''} projects in ${locationLabel.value}.`,
})
</script>
...
<template>
  <div v-if="page" class="container">
    <section class="page-hero">
      <ProjectBreadcrumbs
        :items="[
          ...
          { label: locationLabel, to: page.location.public_path },
    ...
    <section class="cta-band">
      <h2>Need similar work in {{ locationLabel }}?</h2>
