...
const displayBody = computed(() => {
  if (/#/.test(props.body)) return props.body
  const loc = (props.location || 'MetroAtlanta').replace(/[^A-Za-z0-9]+/g, '')
  return `${props.body} #${loc} #painting`
})
const parts = computed(() => captionParts(displayBody.value))
</script>
